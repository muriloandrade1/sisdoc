...
function $merge() {
	...
	for ( var D = 0, A = arguments.length; D < A; D++) {
		...
		for ( var C in B) {
			var G = B[C], F = E[C];
			E[C] = (F && $type(G) == "object" && $type(F) == "object") ? $merge(
					F, G)
					: $unlink(G);
		}
	}
...
var Browser = new Hash({
	...
	Platform : {
		name : (navigator.platform.match(/mac|win|linux/i) || [ "other" ])[0]
				.toLowerCase()
	},
	Features : {
...
Browser.Engine[Browser.Engine.name] = Browser.Engine[Browser.Engine.name
	...
	initialize : function(A, F) {
		...
		if (J.test(/key/)) {
			...
			if (J.match(/(click|mouse|menu)/i)) {
				K = (!K.compatMode || K.compatMode == "CSS1Compat") ? K.html
						: K.body;
				var I = {
				...
				if (J.match(/DOMMouseScroll|mousewheel/)) {
					var H = (A.wheelDelta) ? A.wheelDelta / 120
							: -(A.detail || 0) / 3;
				}
					...
					self[key] = eval("("
							+ String(current)
									.replace(
											/\bthis\.parent\(\s*(\))?/g,
											function(full, close) {
												return "arguments.callee._parent_.call(this"
														+ (close || ", ");
											}) + ")");
				}
...
var Chain = new Class({
	...
	callChain : function() {
		return (this.$chain && this.$chain.length) ? this.$chain.shift().apply(
		...
		.implement({
			...
			set : function(D, B) {
				...
				case "string":
					...
					(A && A.set) ? A.set.apply(this, Array.slice(arguments, 1))
							: this.setProperty(D, B);
				}
				return this;
			},
			get : function(B) {
				var A = Element.Properties.get(B);
				return (A && A.get) ? A.get.apply(this, Array.slice(arguments,
						1)) : this.getProperty(B);
			},
			erase : function(B) {
				var A = Element.Properties.get(B);
				(A && A.erase) ? A.erase.apply(this, Array.slice(arguments, 1))
			...
			clone : function(D, C) {
				...
				case "element":
					...
					for ( var G = 0, E = this.attributes.length; G < E; G++) {
						var B = this.attributes[G], L = B.nodeName
								.toLowerCase();
						...
						var K = (L == "style" && this.style) ? this.style.cssText
								: B.nodeValue;
						if (!$chk(K) || L == "uid" || (L == "id" && !C)) {
							continue;
						}
						if (K != "inherit"
								&& [ "string", "number" ].contains($type(K))) {
							H[L] = K;
						}
					}
			...
			removeClass : function(A) {
				this.className = this.className.replace(
						new RegExp("(^|\\s)" + A + "(?:\\s|$)"), "$1").clean();
				return this;
			},
			toggleClass : function(A) {
				return this.hasClass(A) ? this.removeClass(A) : this
						.addClass(A);
			},
				...
				this
						...
						.each(
								function(B) {
									...
									var C = (B.tagName.toLowerCase() == "select") ? Element
											.getSelected(B).map(function(D) {
												return D.value;
											})
											: ((B.type == "radio" || B.type == "checkbox") && !B.checked) ? null
													: B.value;
									$splat(C)
											.each(
													function(D) {
...
(function() {
	...
	Element
			.implement({
				...
				hasChild : function(B) {
					B = $(B, true);
					return (!!B && $A(this.getElementsByTagName(B.tagName))
							.contains(B));
				}
			});
})();
...
Element.Attributes = new Hash(
		{
			...
			Bools : [ "compact", "nowrap", "ismap", "declare", "noshade",
					"checked", "disabled", "readonly", "multiple", "selected",
					"noresize", "defer" ],
			Camels : [ "value", "accessKey", "cellPadding", "cellSpacing",
					"colSpan", "frameBorder", "maxLength", "readOnly",
		...
		if ($type(A) != "string") {
			...
			A = $splat(A).map(
					function(E, D) {
						...
						return ($type(E) == "number") ? C[D].replace("@", Math
								.round(E)) : E;
					}).join(" ");
		} else {
	...
	getStyle : function(G) {
		...
		if (Browser.Engine.presto
			...
			if (G.test(/^(height|width)$/)) {
				var B = (G == "width") ? [ "left", "right" ] : [ "top",
						"bottom" ], D = 0;
				B.each(function(H) {
					D += this.getStyle("border-" + H + "-width").toInt()
...
(function() {
				...
				getOffsets : function() {
					...
					while (I && !B(I)) {
						...
						if (Browser.Engine.gecko) {
							...
							if (I != this
									&& (Browser.Engine.trident || Browser.Engine.webkit)) {
								H.x += C(I);
								H.y += G(I);
							}
						}
	...
	function A(H) {
		var I = H.getDocument();
		return (!I.compatMode || I.compatMode == "CSS1Compat") ? I.html
				: I.body;
	}
})();
...
var Fx = new Class({
	...
	step : function() {
		...
		if (A < this.time + this.options.duration) {
			var B = this.options.transition((A - this.time)
	...
	search : function(A) {
		...
		Array.each(document.styleSheets, function(E, D) {
			...
			Array.each(F, function(I, G) {
				...
				var H = (I.selectorText) ? I.selectorText.replace(/^\w+/,
						function(J) {
